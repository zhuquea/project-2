...
class OperatingGoodsController extends BaseController {
  ...
  async addShop() {
    ...
    // 购物车已经有了这条商品，商品默认+1
    if (goodsData) {
      await ctx.model.ShopList.findOneAndUpdate(
        { cid: id, uid: _id },
        {
          $set: {
  ...
  async editCart() {
    ...
    await ctx.model.ShopList.findOneAndUpdate(
      { uid: ctx.session.userInfo._id, cid: data.id },
      {
        $set: {
          count: data.count,
  ...
  async deleteShop() {
    ...
    const { ctx } = this;
    await ctx.model.ShopList.deleteMany({
      uid: ctx.session.userInfo._id,
      cid: data
    });
    this.success("删除成功");
  }
  ...
  async address() {
    ...
    if (data.isDefault == true) {
      // 设置默认地址
      await ctx.model.Address.updateMany(
        { uid: _id, isDefault: true },
        {
          $set: {
            isDefault: false
          }
    ...
    if (data.id) {
      ...
      if (addressDef.length == 1) {
        // 如果数据库只有1条，设置这一条为默认地址
        if (!addressDef.isDefault) {
          await ctx.model.Address.findOneAndUpdate(
            { uid: _id, _id: addressDef[0]._id },
            {
              $set: {
                isDefault: true
              }
  ...
  async comment() {
    ...
    // 1，查到对应的订单,直接修改
    await ctx.model.OrderList.findOneAndUpdate(
      {
        uid: userInfo._id,
        order_id: data.order_id,
        "order_list._id": data._id
      },
      {
        $set: {
